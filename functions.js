...
function tester(){
	...
	xhr.open("GET", "tools/usertest.php?login=" + Vid2, true);
...
function recherche(){
	...
	var xhr=getXMLHttpRequest();
	xhr.open("GET", "projet/tools/autocompletion.php?valeur="+valeur, true);
	xhr.onreadystatechange=function()	{
		if(xhr.readyState==4 && (xhr.status==200 || xhr.status === 0)){
			if(xhr.responseText!= ' ')	
...
function panier_udpdate(id_art,nbArticles,j){
	...
	var xhr=getXMLHttpRequest();
	xhr.open("GET", "projet/tools/option_panier.php?id_art="+id_art+"&valeur2="+valeur, true);
	xhr.onreadystatechange=function()	{
		if(xhr.readyState==4 && (xhr.status==200 || xhr.status === 0)){
			if(xhr.responseText!= ' ')	{
